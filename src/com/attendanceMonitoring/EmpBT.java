...
public class EmpBT {
	...
	void printRangePresent(EmployeeNode node, int EmpIdLow, int EmpIdHigh) {
        if (node == null) return;
        if (EmpIdLow < node.getId()) printRangePresent(node.getLeftNode(), EmpIdLow, EmpIdHigh);
        if (EmpIdLow <= node.getId() && EmpIdHigh >= node.getId()) System.out.println(node.getId() + ", " + node.getEmployeeEnteredCount());
        if (EmpIdHigh > node.getId()) printRangePresent(node.getRightNode(), EmpIdLow, EmpIdHigh);
	}
